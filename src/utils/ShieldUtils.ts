...
import { ClientStoppedError, type MatrixClient, type Room } from "../index.web";
import { logger } from "../index.web";
...
export async function shieldStatusForRoom(
	...
	try {
		...
		for (const userId of members) {
			...
			if (
				userTrust.wasCrossSigningVerified() &&
		...
		for (const userId of targets) {
			...
			const anyDeviceNotVerified = await asyncSome(
				devices.keys(),
				async (deviceId) => {
