...
export const concat = (...arrays: Uint8Array[]): Uint8Array => {
	return arrays.reduce(
		(concatenatedSoFar: Uint8Array, toBeConcatenated: Uint8Array) => {
			const concatenated = new Uint8Array(
				concatenatedSoFar.length + toBeConcatenated.length,
			);
...
export async function asyncSomeParallel<T>(
	...
	try {
		return await Promise.any<boolean>(
			values.map((value) =>
				predicate(value).then((result) =>
					result ? Promise.resolve(true) : Promise.reject(false),
				),
			),
		);
	} catch (e) {
