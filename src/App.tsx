...
const EventTile: React.FC<EventTileProp> = ({ item, continuation }) => {
	...
	if (TimelineItemContent.MsgLike.instanceOf(event.item.content)) {
		...
		if (MsgLikeKind.Redacted.instanceOf(message.kind)) {
			...
			if (
				message.kind.inner.content.msgType.inner.content.formatted?.body &&
				message.kind.inner.content.msgType.inner.content.formatted?.format
				...
				body = message.kind.inner.content.msgType.inner.content.body || "";
			}
		}
