...
export class RoomListItemViewModel extends BaseViewModel<
    ...
    { summary: RoomSummary; client: ClientInterface; openRoom: (roomId: string) => void }
> {
    /**
    ...
    public constructor(summary: RoomSummary, client: ClientInterface, openRoom: (roomId: string) => void) {
        ...
        super({ summary, client, openRoom }, {
            ...
            notification: {
                hasAnyNotificationOrActivity: summary.notificationState.hasAnyNotificationOrActivity,
            ...
            canMarkAsRead: summary.unreadMessagesCount > 0 || summary.isMarkedUnread,
            canMarkAsUnread: summary.unreadMessagesCount === 0 && !summary.isMarkedUnread,
    ...
    private async fetchNotificationMode(): Promise<void> {
        try {
            const notificationSettings = await this.props.client.getNotificationSettings();
            const mode = await notificationSettings.getUserDefinedRoomNotificationMode(this.props.summary.id);
            
            this.snapshot.merge({
                roomNotifState: RoomListItemViewModel.mapNotificationModeToState(mode),
            });
        } catch (error) {
    ...
    public onMarkAsRead = async (): Promise<void> => {
        try {
            ...
            console.error(`Failed to mark room ${this.props.summary.id} as read:`, error);
        }
    };
    ...
    public onMarkAsUnread = async (): Promise<void> => {
        try {
            ...
            console.error(`Failed to mark room ${this.props.summary.id} as unread:`, error);
        }
    };

    public onToggleFavorite = async (): Promise<void> => {
        try {
            await this.props.summary.room.setIsFavourite(!this.props.summary.isFavourite, undefined);
        } catch (error) {
            console.error(`Failed to toggle favorite for room ${this.props.summary.id}:`, error);
        }
    };
    ...
    public onToggleLowPriority = async (): Promise<void> => {
        ...
        try {
            ...
            console.error(`Failed to toggle low priority for room ${this.props.summary.id}:`, error);
        }
    };
    ...
    public onCopyRoomLink = async (): Promise<void> => {
        try {
            ...
            console.error(`Failed to copy room link for ${this.props.summary.id}:`, error);
        }
    };
    ...
    public onLeaveRoom = async (): Promise<void> => {
        try {
            ...
            console.error(`Failed to leave room ${this.props.summary.id}:`, error);
        }
    };

    public onSetRoomNotifState = async (state: RoomNotifState): Promise<void> => {
        try {
            const notificationSettings = await this.props.client.getNotificationSettings();

            // AllMessages = "Match default settings" - this should clear the user-defined setting
            if (state === RoomNotifState.AllMessages) {
                await notificationSettings.restoreDefaultRoomNotificationMode(this.props.summary.id);
            } else {
                // Import the RoomNotificationMode enum
                const { RoomNotificationMode } = await import("../generated/matrix_sdk_ffi.ts");
                ...
                await notificationSettings.setRoomNotificationMode(this.props.summary.id, mode);
            }
            ...
            console.error(`Failed to set notification state for room ${this.props.summary.id}:`, error);
        }
    };
}
