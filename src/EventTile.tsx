...
import { VirtualTimelineItem, ProfileDetails, TimelineItemContent, MsgLikeKind, MessageType, MessageFormat_Tags, MembershipChange } from "./index.web";
import { isVirtualEvent, type TimelineItem, type TimelineItemKind } from "./TimelineStore";


export function mxcToUrl(mxcUrl: string): string {
    return (
        mxcUrl.replace(
            /^mxc:\/\//,
...
export const EventTile: React.FC<EventTileProp> = ({ item, continuation }) => {
    ...
    if (TimelineItemContent.MsgLike.instanceOf(event.item.content)) {
        ...
        if (MsgLikeKind.Redacted.instanceOf(message.kind)) {
            ...
            if (message.kind.inner.content.msgType.inner.content.formatted
                ?.body &&
    ...
    } else if (TimelineItemContent.ProfileChange.instanceOf(event.item.content)) {
        const changes: ReactNode[] = [];
        changes.push("changed their ");
        if (event.item.content.inner.avatarUrl !==
            ...
            changes.push([
                ...
                <Avatar
                    ...
                    src={event.item.content.inner.prevAvatarUrl
                ...
                <Avatar
                    ...
                    name={senderProfile.displayName || event.item.sender}
                    id={event.item.sender}
                    src={event.item.content.inner.avatarUrl
                        ? mxcToUrl(event.item.content.inner.avatarUrl)
                        : ""}
                    size="16px" />,
            ]);
            if (event.item.content.inner.displayName !==
        ...
        stateChange = changes;
    } else if (TimelineItemContent.RoomMembership.instanceOf(event.item.content)) {
...
export interface EventTileProp {
