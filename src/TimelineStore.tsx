...
class TimelineStore extends StateStore<TimelineViewState> {
  ...
  onUpdate = (diff: Array<TimelineDiff>): void => {
    ...
    for (const update of diff) {
      //   console.log(
      //     "@@ timelineStoreUpdate",
      //     TimelineDiff[update.change()],
      //     update.change() == TimelineDiff.Set
      //       ? [update.set()!.index, this.parseItem(update.set()?.item)]
      //       : update.change() == TimelineDiff.PushBack
      //       ? this.parseItem(update.pushBack())
      //       : update.change() == TimelineDiff.PushFront
      //       ? this.parseItem(update.pushFront())
      //       : update.change() == TimelineDiff.Clear
      //       ? ""
      //       : update.change() == TimelineDiff.PopFront
      //       ? ""
      //       : update.change() == TimelineDiff.PopBack
      //       ? ""
      //       : update.change() == TimelineDiff.Insert
      //       ? [update.insert()!.index, this.parseItem(update.insert()?.item)]
      //       : update.change() == TimelineDiff.Remove
      //       ? update.remove()
      //       : update.change() == TimelineDiff.Truncate
      //       ? update.truncate()
      //       : update.change() == TimelineDiff.Reset
      //       ? update.reset()!.map(this.parseItem)
      //       : update.change() == TimelineDiff.Append
      //       ? update.append()!.map(this.parseItem)
      //       : "unknown"
      //   );
      switch (update.tag) {
        case TimelineDiff_Tags.Set: {
          newItems[update.inner.index] = this.parseItem(update.inner.value);
          newItems = [...newItems];
          break;
        }
        case TimelineDiff_Tags.PushBack:
          newItems = [...newItems, this.parseItem(update.inner.value)];
          break;
        case TimelineDiff_Tags.PushFront:
          newItems = [this.parseItem(update.inner.value), ...newItems];
          break;
        ...
        case TimelineDiff_Tags.Append:
          newItems = [...newItems, ...update.inner.values.map(this.parseItem)];
          break;
      }
    }
  ...
  run = () => {
    ...
    (async () => {
      ...
      this.pagintationListener = await timeline.subscribeToBackPaginationStatus(
        {
